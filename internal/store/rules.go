...
// NewMemoryRuleStore creates a MemoryRuleStore and starts background cleanup/reset goroutines.
...
func (s *MemoryRuleStore) SetRule(rule *Rule) error {
	if rule.RuleID == "" {
...
func (s *MemoryRuleStore) RemoveRule(ruleID string) (bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *MemoryRuleStore) RemoveAll() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	count := len(s.rules)
...
func (s *MemoryRuleStore) GetStatus() []RuleStatus {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *MemoryRuleStore) ShouldAllow(host, reqPath string) bool {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *MemoryRuleStore) cleanupLoop() {
...
func (s *MemoryRuleStore) deleteExpired() {
	s.mu.Lock()
	defer s.mu.Unlock()
	now := time.Now()
...
func (s *MemoryRuleStore) resetRPMLoop() {
...
func (s *MemoryRuleStore) resetAllRPM() {
	s.mu.RLock()
	defer s.mu.RUnlock()
	for _, rule := range s.rules {
		rule.ResetRPM()
